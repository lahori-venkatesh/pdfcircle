...
import { Upload, Download, Image as ImageIcon, Loader2, X, Crop, RotateCw, Settings2 } from 'lucide-react';
...
export function ImageTools() {
  ...
  const imgRef = useRef<HTMLImage_epresentElement | null>(null);
  ...
  const createPDF = useCallback(async (preview: string, settings: ConversionSettings): Promise<Blob> => {
    ...
    const pdf = new jsPDF({ orientation: 'portrait', unit: 'px' }); // Fixed syntax error
    const img = new Image();
