...
import { Upload, Download, Loader2, X, FileText, FileKey } from 'lucide-react';
  ...
  const handleOCR = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        ...
        const pageImage = await page.render({ scale: 2.0 });
        context.putImageData(pageImage.imageData, 0, 0);
      ...
      // Create new PDF with searchable text
      const newPdfDoc = await PDFDocument.create();
      const textPage = newPdfDoc.addPage([width, height]);
      textPage.drawText(extractedText, {
        x: 50,
        y: height - 50,
        size: 12,
        maxWidth: width - 100,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">How to Use OCR PDF:</h3>
        <ul className="list-disc list-inside text-sm text-gray-600 dark:text-gray-300 space-y-1">
          <li>Upload a scanned PDF document</li>
          <li>Select the language of the text in your document</li>
          <li>Click "Perform OCR" to make the text searchable</li>
