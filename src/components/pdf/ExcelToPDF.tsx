...
export function ExcelToPDF() {
  return (
    <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-6 rounded-lg text-center">
      ...
      <h3 className="text-lg font-semibold mb-2">Excel to PDF Coming Soon!</h3>
      <p>
        We're working on bringing you Excel to PDF conversion capabilities. Stay tuned for updates!
      </p>
    </div>
  );
}
...
export function ExcelToPDF() {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    if (acceptedFiles.length > 1) {
      setError('Please select only one Excel file');
      return;
    }
  ...
  const handleConvert = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:3000/api/excel-to-pdf', {
        method: 'POST',
        headers: {
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <p className="text-gray-600">
          {isDragActive ? 'Drop the Excel file here' : 'Drag & drop an Excel file here, or tap to select'}
        </p>
        <p className="text-sm text-gray-500 mt-2">Supports .xlsx files</p>
      </div>
