...
import { useDOMSafety } from '../utils/domSafety';
...
export function EnhancedImageCrop({ imageSrc, onCropComplete, onCancel, loading = false }: EnhancedImageCropProps) {
  ...
  // Use DOM safety utilities
  const { trackElement, untrackElement, cleanup: cleanupDOM } = useDOMSafety();
  ...
  const { 
    ...
    removeEventListener,
    isMounted 
  } = useComponentLifecycle('EnhancedImageCrop');
  ...
  }, [register, unregister, markUnmounting]);
  ...
  useEffect(() => {
    ...
    if (isDragging || isResizing) {
      // Track elements for safety
      if (imageRef.current) trackElement('image-crop', imageRef.current);
      if (containerRef.current) trackElement('container-crop', containerRef.current);
      
      // Use lifecycle manager for event listeners
      addEventListener(document, 'mousemove', handleGlobalMove, { passive: false });
      addEventListener(document, 'mouseup', handleGlobalEnd);
      addEventListener(document, 'touchmove', handleGlobalMove, { passive: false });
      addEventListener(document, 'touchend', handleGlobalEnd);
      
      // Add cleanup task
      addCleanupTask(() => {
        untrackElement('image-crop');
        untrackElement('container-crop');
      });
    }
  }, [isDragging, isResizing, handleMove, handleEnd, trackElement, untrackElement, addEventListener, addCleanupTask, isMounted]);
  ...
  }, [isMounted, addCleanupTask]);

  // Component cleanup on unmount
  useEffect(() => {
    return () => {
      // Clean up DOM tracking
      cleanupDOM();
    };
  }, [cleanupDOM]);
